...
export default function AccountShortcuts() {
  const depositIcon = (
    <svg
      ...
      fill=""
      viewBox="0 0 24 24"
      className="w-5 h-5 fill-svgFill"
    >
  ...
  return (
    <div className="border border-borderColor rounded-md flex flex-col p-3 gap-3 lg:col-start-3 col-span-1 h-fit">
      <p className="text-lightTextColor font-medium hidden lg:flex flex-col">
        Shortcuts
      </p>

      <div className="flex flex-row justify-between lg:flex-col lg:justify-start">
        {shortcutLinks.map((link, index) => (
          <Button
            variant="ghost"
            className="flex items-center px-2 flex-col h-fit gap-1 justify-center lg:flex-row lg:justify-start lg:gap-3 text-lightTextColor hover:bg-lightBackground hover:text-lightTextColor"
            key={index}
          >
            {link.shortcutIcon}
            {link.text}
